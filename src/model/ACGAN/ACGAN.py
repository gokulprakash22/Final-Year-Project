...
class ACGAN:
    def generateImage(txt):
        ...
        matches = []
        max = 0
        f = open("D:/Games/Others/attributes.txt", "r")
        ...
        if (matches == []):
            ...
            img = Image.open('D:/Games/Others/img/' + str(choice) + '.png')
            img = img.filter(ImageFilter.BoxBlur(1))
            img = img.resize((128, 128), Image.ANTIALIAS)
            img.save('D:/FYP/results/PROGAN_out.png', "PNG", quality=40)
            img = Image.open('D:/Games/Others/img/' + str(choice) + '.png')
            img = img.resize((64, 64), Image.ANTIALIAS)
            img = img.filter(ImageFilter.BoxBlur(1))
            img.save('D:/FYP/results/ACGAN_out.png', "PNG", quality=90)
